...
long timeInMilliseconds(void) {
    ...
    return (((long) tv.tv_sec) * 1000) + (tv.tv_usec);
}
...
ffdotpows *subharm_fderivs_vol(int numharm, int harmnum,
    ...
    {
#ifdef CLFFT
        size_t clLengths[1] = {fftlen};
        err = clfftCreateDefaultPlan(&planHandle, context, CLFFT_1D, clLengths);
        if (err < 0) {
            printf("create cl default plan error: %d \n", err);
        }

        err = clfftSetPlanPrecision(planHandle, CLFFT_SINGLE);
        err = clfftSetLayout(planHandle, CLFFT_COMPLEX_INTERLEAVED, CLFFT_COMPLEX_INTERLEAVED);
        err = clfftSetResultLocation(planHandle, CLFFT_INPLACE);
        err = clfftSetPlanScale(planHandle, CLFFT_BACKWARD, 1.0f);

        if (err < 0) {
            printf("set plan paramters error: %d \n", err);
        }
...
#pragma GCC ivdep
...
#ifdef CLFFT
                timeCounter = timeInMilliseconds();

                float *in_local_buffer = (float *) malloc(fftlen * 2 * sizeof(float));
                float *out_local_buffer = (float *) malloc(fftlen * 2 * sizeof(float));

                for (int i = 0; i < fftlen; i++) {
                    in_local_buffer[2 * i] = tmpdat[i].r;
                    in_local_buffer[2 * i + 1] = tmpdat[i].i;
                }


                cl_mem inBuffer = clCreateBuffer(
                        context,
                        CL_MEM_READ_WRITE,
                ...
                if (err < 0) {
                    printf("cl write buffer error: %d \n", err);
                }
                ...
                if (err < 0) {
                    printf("cl fft enqueue transform error: %d \n", err);
                }

                /* Wait for calculations to be finished. */
                err = clFinish(queue);
                if (err < 0) {
                    printf("cl wait for queue error: %d \n", err);
                }
...
//                printf("cl fft kernel finished\n");

                for (int iter = 0; iter < fftlen; iter++) {
                    tmpout[iter].r = out_local_buffer[2 * iter];
                    tmpout[iter].i = out_local_buffer[2 * iter + 1];
                }

                free(in_local_buffer);
                free(out_local_buffer);
                err = clReleaseMemObject(inBuffer);
                if (err < 0) {
                    printf("cl realse buffer error: %d \n", err);
                }

                printf("cost time : %ld \n", timeInMilliseconds() - timeCounter);
