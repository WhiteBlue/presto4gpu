...
int main(int argc, char *argv[]) {
    ...
    //    /* Build program */
    //    program = build_program(context, device, "kernel.cu");
    ...
    double time_single_step = times(&runtimes) / (double) CLK_TCK;

    // 读取文件创建结构，I/O操作
    /* Create the accelobs structure */
    create_accelobs(&obs, &idata, cmd, 1);

//    printf("============ [I/O] single step time: %.3f ==========",
//           (times(&runtimes) / (double) CLK_TCK) - time_single_step);
    ...
    /* Start the main search loop */
    {
        ...
        time_single_step = times(&runtimes) / (double) CLK_TCK;
        ...
        if (obs.inmem) {
            ...
            while (startr < obs.rlo) {
                ...
                fundamental = subharm_fderivs_vol(1, 1, startr, lastr,
                                                  &subharminfs[0][0], &obs, context, queue);
...
//        printf("============ [Populate] single step time: %.3f ==========",
//               (times(&runtimes) / (double) CLK_TCK) - time_single_step);
...
//            printf("---------- [subharm_fderivs_vol] loop time: %.3f ----------",
//                   (times(&runtimes) / (double) CLK_TCK) - time_single_step);
...
//            printf("---------- [search_ffdotpows] loop time: %.3f ----------",
//                   (times(&runtimes) / (double) CLK_TCK) - time_single_step);
...
//            printf("---------- [search_the_subharmonics] loop time: %.3f ----------",
//                   (times(&runtimes) / (double) CLK_TCK) - time_single_step);
